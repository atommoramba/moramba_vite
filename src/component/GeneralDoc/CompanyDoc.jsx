...
dayjs.extend(utc);
function CompanyDoc() {
  //Redux code from CompanyDocSlice
  ...
  const fetchAllCompanyDoc = () => {
    setIsLoading(true);

    var companydoclist = [];
    for (var i = 0; i < CompanyDocData.length; i++) {
      setTemplateId(CompanyDocData[0].templateid);

      companydoclist.push({
        _id: CompanyDocData[i]._id,
        category: CompanyDocData[i].category,
        active: CompanyDocData[i].active.toString(),
        t_id: CompanyDocData[i].t_id,
      });
    }
    Promise.all([setDocList(companydoclist)]).then(() => {
      setTimeout(() => {
        setIsLoading(false);
      }, 1500);
    });
  ...
  const onFilterChange = (type, newDoc, event = null, index) => {
    ...
    var data = { _id: compid, companydoclist: [] };
  ...
  const companydocHandler = () => {
    if (textinputenter !== "") {
      ...
      var listTemp = doclist;
      setDocList([...listTemp, o]);
      setAddData(false);
      setTextinputenter("");
    } else {
  ...
  const saveData = () => {
    ...
    var API_URL =
      ...
      "/API/moramba/v3/crud/collection/mcompanydoccategorylist?type=update";
    let headerConfig = {
      headers: {
  ...
  return (
    <>
      <div className="containerBox  company_data mt-1" id="mob_company">
        <h3 className="mt-3 mb-3 HeadingText text-center">
        ...
        <input
          type="checkbox"
          className="CheckboxDoc mx-5 markmob"
          id="check-all"
          value="Mark all"
            ...
            <div className="doc_data_scroll" id="scroll_mobile">
              <div className="d-flex flex-wrap justify-content-start">
                {doclist.map((item, i) => {
                  return (
                    <>
                      <div className="mainboxcheckboxsty categ_mob">
                        <input
                          type="checkbox"
                          ...
                          name={item.category}
                        />{" "}
                        <span id="mobcheck">{item.category}</span>
                      </div>
                    </>
                  );
                })}
