...
function InventoryTemplateList() {
  //Redux code from InventoryTemplateSlice
  const dispatch = useDispatch();
  const InventoryData = useSelector((state) => state.InventoryTemplate);
  ...
  const [title_InvenTemp, setTitle_InvenTemp] = useState("Inventory Templates");
  ...
  useEffect(() => {
    if (InventoryData?.length === 0) {
      ...
      Promise.all([dispatch(getInventoryTemplate())]).then(() =>
        setTimeout(() => {
          setIsLoading(false);
        }, 1000)
      );
  ...
  const columns = [
    ...
    },
    {
      name: <>{btn_view}</>,
      selector: (row) => (
        <Link to="/inventory/template/view" state={{ data: row }}>
    ...
    <>
      ...
      <div className="container containerBox mt-4 p-2">
        <div className="d-flex justify-content-end">
          ...
          &nbsp;&nbsp;
          <Link to="/inventory/template/create">
...
export default InventoryTemplateList;
