...
function WeeklyTimesheet() {
  ...
  const weeklySheetData = useSelector((state) => state.WeeklyTimesheet);
  console.log(weeklySheetData);
  //language variable
  ...
  useEffect(() => {
    ...
    if (
      weeklySheetData?.length === 0 ||
      empId !== weeklySheetData[0]?.employeeId
    ) {
      setIsLoading(true);

      console.log("invoice Data API CALLED");
      Promise.all([dispatch(getWeeklyTimesheet())]).then(() =>
        setTimeout(() => {
          setIsLoading(false);
        }, 1500)
      );
  ...
  const filteredItems = weeklySheetData?.filter(
    (item) =>
    ...
    <>
      <div className="container mt-4">
        <div className="d-flex justify-content-end">
          ...
          &nbsp;&nbsp;
          <Link to="/addtimesheet" state={{ data: 1 }}>
